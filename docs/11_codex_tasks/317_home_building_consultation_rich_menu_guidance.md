...
Implement the next customer rich-menu flow one item at a time: tapping `家づくり相談`
should guide the customer to the consultation page and record sanitized CRM evidence that the page
was 案内済み.

## Scope

- Change only the initial/default customer rich menu `家づくり相談` area from a direct URI action to a
  message action.
- Handle the exact message text `家づくり相談` in the LINE webhook through the existing guide-action
  path.
- Reply with a short consultation guide containing the public consultation URL.
- Record `家づくり相談ページ案内済み` in the customer timeline.
- Do not create an unreplied staff alert for this guide-only action.

## Result

- Added `initial.home_building_consultation` to the domain guide-action resolver.
- Reused the existing webhook reply handling for customer rich-menu guide actions.
- Home-building consultation guide taps create a system timeline record with the public consultation
  URL as source metadata.
- The customer-facing rich menu definition now routes the consultation tile through LINE message

...
Note: this original dry-run planning doc was followed by a candidate-only extension in [112_nginx_reverse_proxy_dry_run_and_sites_available_candidate.md](112_nginx_reverse_proxy_dry_run_and_sites_available_candidate.md). The extension placed `/etc/nginx/sites-available/amami-line-crm.conf` on the VPS and ran `sudo nginx -t`, while still avoiding `sites-enabled`, Nginx reload/restart, certbot, DNS changes, and public exposure.

...
Create the operator-side restore execution followup after Loop 277 recorded the controlled restore retry result as `not_attempted`.

This Loop clarifies that the next action is an operator-side execution approval decision, not Codex direct restore execution. It does not run restore, restore retry, `pg_restore`, `psql`, Supabase connection, DB/schema/role/extension/cluster changes, package operations, infrastructure changes, LINE sends, OpenAI calls, runtime changes, or artifact/raw secret disclosure.
...
Loop 277 recorded a sanitized operator-side restore result as `not_attempted`. No restore retry ran, so DR readiness remains incomplete.
...
## Operator-Side Execution Followup

```txt
purpose=operator-side restore retry execution followup
execution_mode=operator_side_only
codex_direct_restore_execution_allowed=false
codex_direct_db_access_allowed=false
...
Operator-side execution steps must stay category-only:
...
## Approval Block Required Before Actual Restore Execution
...
| Operator-side execution followup | Prepared | Followup runbook and approval blocks are ready. |
| Restore execution in Loop 278 | No-Go | Actual restore requires a separate operator approval decision. |
| Codex direct restore/DB access | No-Go | Still prohibited. |
| Retry without new approval | No-Go | Still prohibited. |

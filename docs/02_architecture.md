...
Loop 018のalert UIでは、`/alerts` から未返信チェック、alert一覧、open alertのMock通知を開発確認できます。alert一覧は `GET /api/admin/alerts` でtenant scopedに取得し、notify-openはAPI側の `MockStaffNotifier` 境界を前提にします。本番LINEグループ通知はまだ接続しません。

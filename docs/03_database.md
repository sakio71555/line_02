...
Loop 086では、Loop 085でstaging拡張検証版100%相当に到達した後のproduction hardeningを分割しました。Loop 093でproduction dev_header rejectionは実装済みですが、productionは引き続きNo-Goで、RLS未実装、Auth/JWT本接続未完了、Admin UI selectedTenantId保存未実装、LINE/OpenAI real provider未接続を明確に残しています。

Loop 086ではmigration SQL unchanged / no RLS SQLを守り、RLS SQL、Auth/JWT、API runtime、UI、Supabase接続は変更していません。Loop 093の詳細は [docs/11_codex_tasks/093_production_dev_header_rejection_auth_jwt_boundary.md](11_codex_tasks/093_production_dev_header_rejection_auth_jwt_boundary.md) を参照してください。
...
Loop 096ではstaging DB上で `SET LOCAL ROLE authenticated` と `request.jwt.claim.sub` によるdummy `auth.uid()` simulationを使い、RLS policyのtenant A/B分離を確認しました。active staff + active membershipでは同tenantのcustomers/messages/alerts/allowed knowledgeが読め、wrong tenant、inactive staff、inactive membership、`knowledge_pages.allowed_for_ai=false` は読めないことを確認済みです。write smokeは `BEGIN ... ROLLBACK` 内のdummy rowsだけで確認しました。本物Supabase Auth user作成、Supabase Auth/JWT本接続、production接続は未実施です。詳細は [docs/11_codex_tasks/096_authenticated_role_jwt_rls_smoke.md](11_codex_tasks/096_authenticated_role_jwt_rls_smoke.md) を参照してください。
...
Loop 108ではVPS dry deployment preflight command pack、rollback、No-Go checklistを追加しました。DB schema、RLS SQL、migration、GRANT、Supabase接続は変更していません。VPS実作業、production Supabase接続、LINE/OpenAI real smokeは未実施で、production readinessは引き続き `production_no_go` です。詳細は [docs/15_runbooks/vps_dry_deployment_preflight_commands.md](15_runbooks/vps_dry_deployment_preflight_commands.md) を参照してください。

Loop 109ではVPS localhost-only mock deploymentを実施しますが、`REPOSITORY_RUNTIME=in_memory` のreview/mock起動に限定します。DB schema、RLS SQL、migration、GRANT、Supabase production/staging接続は変更せず、production readinessは引き続き `production_no_go` です。詳細は [docs/15_runbooks/vps_localhost_mock_deployment_execution.md](15_runbooks/vps_localhost_mock_deployment_execution.md) を参照してください。
...
## Production hardening links

RLS/Auth/JWT production hardeningは小さいLoopに分割して進めます。Loop 086ではproduction hardening split planをdocs-onlyで整理し、Loop 093ではproduction modeのdev header rejectionだけを実装しました。

Loop 094Aでは `packages/db/migrations/0003_rls_core_tables.sql` をRLS SQL draftとして追加しました。対象は `tenants`、tenant設定、customers/messages/alerts/knowledge、staff identity/membershipのcore tablesです。policyは `auth.uid()::text`、`staff_users.auth_user_id`、active `staff_users`、active `staff_tenant_memberships` を使う方針で、`anon` へのgrant/policy、`using true`、`with check true`、`grant all on all tables` を静的検証で禁止しています。staging applyはまだ未実施で、production readinessはNo-Go継続です。
...
Loop 105ではAdmin UI login/session minimal integrationを追加しました。Admin session controllerはfake Supabase auth clientでsessionを読み、access tokenをsafe resultへ含めず、Admin API helper向けtoken providerから都度取得する境界だけを提供します。`selectedTenantId` は非secret selectorとして分離し、DB/repositoryで使うtenantは引き続きAPI側active membership再検証後に確定します。実Supabase Auth client注入、production接続、DB schema変更は未実施です。

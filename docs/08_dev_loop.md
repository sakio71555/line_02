...
本番認証、JWT検証、staff/admin tenant context、RLSのようなセキュリティ境界は、実装Loopの前に必ずplanning Loopを挟みます。開発用 `x-tenant-id` は本番認証ではないため、認証済みuser/sessionからtenant contextを決定する設計を先に固めてからAPI差し替えへ進みます。

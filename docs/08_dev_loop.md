...
本番tenant contextは、`schema -> auth context boundary -> API tenant guard -> RLS/local test -> runtime接続` の順に分けて進めます。boundary Loopではpure resolverとfake lookup testに留め、既存API routeへ接続しません。

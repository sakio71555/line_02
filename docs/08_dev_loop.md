...
Loop 194.1のようなSupabase manual backup availability result after Free Plan limitationでは、operatorが外部で確認したsanitized resultだけを記録します。Free Plan limitationでmanual/managed backupが使えない場合は、`manual_backup_available=false`、`managed_backup_available=false`、`backup_status=not_performed`、`backup_success_recorded=false` とし、次Loopでbackup path decisionを行います。CodexはSupabase dashboard操作、CLI/API、DB export、restore、artifact downloadを行いません。
...
Loop 202 pg_dump 17 client boundaryのようなbackup mismatch記録Loopでは、raw log、DB URL、secret、artifact内容を表示せず、sanitizedなversion mismatchだけを記録します。`pg_dump_failure_categories=pg_dump_server_version_mismatch`、`detected_server_major_or_version=17.6`、`detected_pg_dump_major_or_version=16.14`、`raw_log_not_displayed=true`、`secrets_recorded=false` を固定し、PostgreSQL 17 client導入前のbackup export再実行を禁止します。PostgreSQL client 17導入は別Loopに分離し、同じLoopでexportやrestoreを行いません。

## Obsidian Development Log Requirement

Every Loop must update Obsidian-readable Markdown logs:

- `docs/14_dev_logs/<date>.md`
- `docs/11_codex_tasks/<loop>.md` when applicable
- `docs/15_runbooks/<runbook>.md` when applicable
- `OBSIDIAN.md` or `docs/16_obsidian` index files when navigation changes

Codex final reports should include:

```txt
Obsidian log updated: true/false
Obsidian entry/index updated: true/false
DevelopmentLog updated: true/false
```

The repo root may be opened as an Obsidian Vault, but Markdown in Git remains the source of truth. Obsidian workspace/cache/plugin/theme files are local personal state and should not be committed.

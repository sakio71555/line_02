...
import { getAdminAuditEvents, getAdminReplyTemplates, getAdminStaffDirectory, getAdminWorkspaceSettings } from "../../src/admin-api";
...
export default async function SettingsPage() {
  ...
  const [settingsResult, templatesResult, auditResult, staffResult] = await Promise.allSettled([
    getAdminWorkspaceSettings(options), getAdminReplyTemplates(options), getAdminAuditEvents(30, options), getAdminStaffDirectory(options)
  ]);
  ...
  const canManageStaff = staffResult.status === "fulfilled";
  ...
  return <main><PageTitle eyebrow="管理画面設定" title="設定" description="会社の表示、対応ルール、返信定型文、操作履歴を管理します。" />
    ...
    {!settings ? <div className="inline-error">保存済み設定を読み込めませんでした。</div> : <SettingsWorkspace auditEvents={auditEvents} canManageStaff={canManageStaff} initialSettings={settings} settingsVersion={settingsVersion} templates={templates} />}
  </main>;
}
